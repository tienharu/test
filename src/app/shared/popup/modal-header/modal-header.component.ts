...
export class ModalHeaderComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    widget.className += ' jarviswidget jarviswidget-sortable';
    // if (this.sortable) {
    //   widget.className += ' jarviswidget-sortable';
    // }

    if (this.color) {
      widget.className += (' jarviswidget-color-' + this.color+' jarviswidget-' + this.color);
    }
  ...
  private genId() {
    if (this.name) {
      ...
      let id = heading ? heading.textContent.trim() : 'jarviswidget-' + ModalHeaderComponent.counter++;
