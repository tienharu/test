...
@Component({
  selector: 'sa-sub-material',
  templateUrl: './sub-material.component.html',
  styleUrls: ['./sub-material.component.css'],
  entryComponents: [
    CustomRenderSmartTableStyleInputComponent, CustomRenderSmartTableStyleInputCurrencyComponent, CustomRenderSmartTableStyleSelect2Component,
    CustomRenderSmartTableStyleInputFloatComponent
  ]
})
export class SubMaterialComponent extends BasePage implements OnInit {
  ...
  private initTable() {
    ...
    self.subSettings = {
      ...
      attr: {
        class: 'table table-bordered fixed_header sub-material-table'
      },
    ...
    setTimeout(function(){
      if(!self.styleSysId){
        $('.sub-material-table .ng2-smart-filters').hide();
      }else{
        $('.sub-material-table .ng2-smart-filters').show();
      }
    },10);
   
  }

  onSubCreateConfirm(event) {
    var self = this;
  ...
  onSubDeleteConfirm(event) {
    var self = this;
    var data = event.data;
    self.notification.smartMessageBox(
      {
        title: "<i class='fa fa-remove txt-color-orangeDark'></i> Delete Sub Material ?",
        content: "Are you want to delete this sub material?",
        buttons: "[No][Yes]"
      },
      ButtonPressed => {
  ...
  private loadMaterialList() {
    ...
    if (self.styleSysId && self.styleSysId !== 0) {
      return self.styleMasterService.getListStyleMasterMaterialByStyle(self.styleSysId).then(function (rs) {
        if (rs.success) {
          self.data = _.map(rs.data && rs.data.listSubMaterials, function (item, index) {
            item.index = index;
