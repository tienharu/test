...
@Component({
  selector: 'sa-work-order-row-material',
  templateUrl: './work-order-row-material.component.html',
  styleUrls: ['../../../../../../assets/css/smart-table.scss', './work-order-row-material.component.css'],
  entryComponents: [
    CustomRenderSmartTableWorkOrderInputComponent, CustomRenderSmartTableWorkOrderInputCurrencyComponent, CustomRenderSmartTableWorkOrderSelect2Component, CustomRenderSmartTableWorkOrderInputFloatComponent
  ]
})

export class WorkOrderRowMaterialComponent extends BasePage implements OnInit {
  ...
  private initTable() {
    this.rowSettings = {
      ...
      attr: {
        class: 'table table-bordered fixed_header work-order-row-material-table'
      },
      noDataMessage: this.i18nService.getTranslation('sEmptyTable')
    };
    this.rowSource.load(this.data || []);
    setTimeout(function () {
      $('.work-order-row-material-table .ng2-smart-filters').hide();
    }, 100);
  }
  ...
  private loadMaterialList() {
    ...
    if (self.styleSysId && !self.woNo && !self.stepSeq) {
      return self.styleMasterService.getListStyleMasterMaterialByStyle(self.styleSysId).then(function (rs) {
        if (rs.success) {
          self.data = _.map(rs.data && rs.data.listRowMaterials, function (item) {
            if (self.colors && self.colors[item.colorIdStyle]) {
              item.materialQty = self.colors[item.colorIdStyle] && self.colors[item.colorIdStyle].total;
            }
            return self.getMoreInfo(item);
