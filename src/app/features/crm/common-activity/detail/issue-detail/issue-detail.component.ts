...
@Component({
  selector: 'sa-issue-detail',
  templateUrl: './issue-detail.component.html',
  styleUrls: ['../../common-activity.css']
})
export class IssueDetailComponent extends BasePage implements OnInit {
  ...
  getIssueInfo(companyId,inputFuncId,funcRefCd) {
    ...
    this.crmActivityIssueService.getIssueInfo(companyId,inputFuncId,funcRefCd).then(data => {
      this.issueInfo = data.data;
      this.issueInfo.forEach(element => {
  ...
  reloadData(fromChild) {
    this.getIssueInfo(this.companyId, this.inputFuncId, this.funcRefCd)
